...
type PnPickerTarget = "parceiro" | "responsavel";
...
export function VisitaFormPage({ visitaId }: { visitaId?: number }) {
  ...
  const [responsaveis, setResponsaveis] = useState<ParceiroOption[]>([]);
  ...
  const bindParceiroOption = (option: ParceiroOption) => {
    if (pnPickerTarget === "parceiro") {
      ...
      setResponsaveis((prev) => upsertParceiroOption(prev, option));
    }
    closePnPicker();
  };
  ...
  const saveVisita = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        tipoContratoSugerido: form.tipoContratoSugerido,
        atividades,
        categoriaItens,
      };
  ...
  const gerarContrato = async () => {
    ...
    try {
      const r = await fetch(`/api/visitas/${visitaId}/gerar-contrato`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tipoContrato: form.tipoContratoSugerido }) });
    ...
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        ...
        <section className="card p-3">
          <div className="legacy-actions"><button type="button" className="legacy-btn primary" onClick={saveVisita} disabled={saving || loading}>{saving ? "Salvando..." : "Salvar"}</button><Link href="/visitas" className="legacy-btn">Descartar</Link>{visitaId && <button type="button" className="legacy-btn" onClick={gerarContrato} disabled={generating || loading}>{generating ? "Gerando..." : "Gerar Contrato"}</button>}</div>
          ...
          <div className="legacy-grid cols-4 mt-2">
            ...
            <FieldSelect label="Tipo Contrato Sugerido" value={form.tipoContratoSugerido} onChange={(v) => setForm((p) => ({ ...p, tipoContratoSugerido: v === "saida_insumos" ? "saida_insumos" : "entrada_animais" }))} options={[{ value: "entrada_animais", label: "Entrada de Animais" }, { value: "saida_insumos", label: "Saida de Insumos" }]} />
            ...
            <FieldInput label="CEP" value={form.cep} onChange={(v) => setForm((p) => ({ ...p, cep: v }))} />
            ...
            <FieldInput label="Categoria" value={form.categoria} onChange={(v) => setForm((p) => ({ ...p, categoria: v }))} />
            <FieldInput label="Raca" value={form.raca} onChange={(v) => setForm((p) => ({ ...p, raca: v }))} />
          ...
          {tab === "atividade" && (
            <section className="mt-2">
              <div className="legacy-grid cols-4">
                ...
                <FieldSelect label="Responsavel" value={atividadeDraft.responsavel} onChange={(v) => setAtividadeDraft((p) => ({ ...p, responsavel: v }))} options={[{ value: "", label: "Selecione..." }, ...responsaveis.map((o) => ({ value: labelOption(o), label: labelOption(o) }))]} />
          ...
          {tab === "categoria" && (
            <section className="mt-2">
              <div className="legacy-grid cols-4">
                <FieldInput label="Categoria" value={categoriaDraft.categoria} onChange={(v) => setCategoriaDraft((p) => ({ ...p, categoria: v }))} />
                <FieldInput label="Raca" value={categoriaDraft.raca} onChange={(v) => setCategoriaDraft((p) => ({ ...p, raca: v }))} />
