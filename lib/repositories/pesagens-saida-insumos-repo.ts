...
export async function listPesagensSaidaInsumos(filters: PesagemEntradaAnimaisListFilters) {
  ...
  const [countResult, rowsResult] = await Promise.all([
    pool.query<{ total: string }>(
      ...
      WHERE lower(coalesce(p.tipo, '')) = 'saida_insumos'
...
export async function getPesagemSaidaInsumosById(id: number): Promise<PesagemEntradaAnimaisRecord | null> {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
  ...
  const [baseResult, docsResult, atrasoResult, esperaResult, calendarioResult, gtaResult, classificacaoResult, fechamentoResult] = await Promise.all([
    pool.query(
      ...
      FROM ${PESAGEM_TABLE} p
      WHERE p.id = $1
        AND lower(coalesce(p.tipo, '')) = 'saida_insumos'
      `,
      [id],
    ),
...
export async function createPesagemSaidaInsumos(input: PesagemEntradaAnimaisCreateInput) {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
  ...
  try {
    ...
    return await getPesagemSaidaInsumosById(id);
  } catch (error) {
...
export async function updatePesagemSaidaInsumos(id: number, input: PesagemEntradaAnimaisUpdateInput) {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
...
export async function listPesagemSaidaInsumosOptions(filters?: {
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS ${PESAGEM_TABLE} (
        ...
        tipo VARCHAR(32) NOT NULL DEFAULT 'saida_insumos',
...
function normalizeTipo(value: unknown): PesagemTipo {
  ...
  if (normalized === "saida_insumos") return "saida_insumos";
