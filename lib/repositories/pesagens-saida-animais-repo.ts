...
import type {
  ...
  PesagemSaidaAnimaisCreateInput,
  PesagemSaidaAnimaisListFilters,
  PesagemSaidaAnimaisOptionsPayload,
  PesagemSaidaAnimaisRecord,
  PesagemFechamento,
  PesagemGtaRow,
  PesagemSaidaAnimaisUpdateInput,
  PesagemMotivoRow,
  PesagemStatus,
  PesagemTipo,
...
export async function listPesagensSaidaAnimais(filters: PesagemSaidaAnimaisListFilters) {
  ...
  const [countResult, rowsResult] = await Promise.all([
    pool.query<{ total: string }>(
      ...
      WHERE lower(coalesce(p.tipo, '')) = 'saida_animais'
...
export async function getPesagemSaidaAnimaisById(id: number): Promise<PesagemSaidaAnimaisRecord | null> {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
  ...
  const [baseResult, docsResult, atrasoResult, esperaResult, calendarioResult, gtaResult, fechamentoResult] = await Promise.all([
    pool.query(
      ...
      FROM ${PESAGEM_TABLE} p
      WHERE p.id = $1
        AND lower(coalesce(p.tipo, '')) = 'saida_animais'
      `,
      [id],
    ),
...
export async function createPesagemSaidaAnimais(input: PesagemSaidaAnimaisCreateInput) {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
  ...
  try {
    ...
    return await getPesagemSaidaAnimaisById(id);
  } catch (error) {
...
export async function updatePesagemSaidaAnimais(id: number, input: PesagemSaidaAnimaisUpdateInput) {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
...
export async function listPesagemSaidaAnimaisOptions(filters?: {
  contratoSearch?: string | null;
  contratoLimit?: number | null;
}): Promise<PesagemSaidaAnimaisOptionsPayload> {
  const pool = getPgPool();
  await ensurePesagemSchema(pool);
  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS ${PESAGEM_TABLE} (
        ...
        tipo VARCHAR(32) NOT NULL DEFAULT 'saida_animais',
...
function normalizeTipo(value: unknown): PesagemTipo {
  ...
  if (normalized === "saida_animais") return "saida_animais";
  return "saida_animais";
}
