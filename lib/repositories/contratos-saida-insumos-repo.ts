...
import type {
  ...
  ContratoSaidaInsumosCreateInput,
  ContratoSaidaInsumosStatusChangeInput,
  ContratoSaidaInsumosUpdateInput,
  ContratoStatus,
...
export async function listContratosSaidaInsumos(filters: ListFilters) {
...
export async function getContratoSaidaInsumosById(id: number) {
  const pool = getPgPool();
  await assertContratoTable(pool);
...
export async function createContratoSaidaInsumos(input: ContratoSaidaInsumosCreateInput) {
  const pool = getPgPool();
  await assertContratoTable(pool);
  ...
  try {
    ...
    return await getContratoSaidaInsumosById(contratoId);
  } catch (error) {
...
export async function updateContratoSaidaInsumos(id: number, input: ContratoSaidaInsumosUpdateInput) {
  const pool = getPgPool();
  await assertContratoTable(pool);
  ...
  try {
    ...
    await client.query("COMMIT");
    return await getContratoSaidaInsumosById(id);
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
...
export async function changeContratoSaidaInsumosStatus(id: number, input: ContratoSaidaInsumosStatusChangeInput) {
  const pool = getPgPool();
  await assertContratoTable(pool);
...
async function assertContratoTable(db: Queryable) {
  ...
  if (csExists) {
    throw new Error("Encontrada tabela public.cs_contrato, mas o modulo Saida de Insumos esta configurado para o esquema contrato.*.");
  }
