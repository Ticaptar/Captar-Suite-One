...
export default function NovoContratoEntradaAnimaisPage() {
  ...
  useEffect(() => {
    ...
    async function loadContrato() {
      ...
      setSuccess("");
      try {
        const response = await fetch(`/api/contratos/entrada-animais/${editingContratoId}`, {
          cache: "no-store",
        });
  ...
  async function handleSaveContract(options?: { gerarPdf?: boolean }) {
    ...
    const endpoint = currentContratoId
      ? `/api/contratos/entrada-animais/${currentContratoId}`
      : "/api/contratos/entrada-animais";
    ...
    try {
      ...
      if (contratoId) {
        ...
        router.replace(`/contratos/entrada-animais/novo?id=${contratoId}`);
        if (options?.gerarPdf) {
          window.open(`/api/contratos/entrada-animais/${contratoId}/pdf`, "_blank", "noopener,noreferrer");
          return;
        }
        return;
      }
  ...
  return (
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        <FormPageHeader
          title={isEditMode ? "Editar Contrato de Entrada de Animais" : "Novo Contrato de Entrada de Animais"}
          ...
          <div className="legacy-toolbar">
            <div className="legacy-toolbar-left">
              <h1 className="legacy-title">
                Contrato de Entrada de Animais / {isEditMode ? `(ID ${editingContratoId})` : "(Novo)"}
              </h1>
              <div className="legacy-actions">
                ...
                <Link href="/contratos/entrada-animais" className="legacy-btn">Descartar</Link>
                ...
                <button
                  ...
                  onClick={() => {
                    if (savedContratoId) {
                      window.open(`/api/contratos/entrada-animais/${savedContratoId}/pdf`, "_blank", "noopener,noreferrer");
                      return;
                    }
          ...
          <div className="legacy-form mt-2">
            <div className="legacy-grid cols-4">
              <CatalogAutocompleteField
                ...
                listId="empresa-entrada-animais"
            ...
            {activeTab !== "dados" && activeTab !== "itens" && activeTab !== "frete" && activeTab !== "financeiro" && activeTab !== "clausulas" && activeTab !== "dados_gerais" && activeTab !== "custos" && activeTab !== "outros" && (
              <TabTable
              ...
              <div className="legacy-grid cols-4 mt-2 dados-gerais-lock">
                ...
                <label className="legacy-field" data-editavel="true">
                  ...
                  <div className="legacy-inline">
                    <input
                      className="legacy-input"
                      value={optionLabel(parceiroCatalogOptions, dadosGerais.originador)}
                      placeholder="Nenhum selecionado"
                      readOnly
                    />
                    <button type="button" className="legacy-btn" onClick={() => openPnPicker("originador")}>
                      Pesquisar
                    </button>
                  </div>
                  {loadingParceiros && <small className="mt-1 block text-xs text-[#6b7394]">Carregando opções...</small>}
                </label>
                <label className="legacy-field" data-editavel="true"><span>Sexo</span><select className="legacy-select" value={dadosGerais.sexo} onChange={(event) => setDadosGerais((prev) => ({ ...prev, sexo: event.target.value }))}><option value="">Selecione...</option><option value="Macho">Macho</option><option value="Fêmea">Fêmea</option><option value="Misto">Misto</option></select></label>
                <label className="legacy-field" data-editavel="true"><span>Quantidade Negociada</span><input className="legacy-input" value={dadosGerais.quantidadeNegociada} onChange={(event) => setDadosGerais((prev) => ({ ...prev, quantidadeNegociada: onlyDecimal(event.target.value) }))} /></label>
                ...
                <label className="legacy-field"><span>Animais Desembarcados</span><input className="legacy-input" value={dadosGerais.animaisDesembarcados} onChange={(event) => setDadosGerais((prev) => ({ ...prev, animaisDesembarcados: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field"><span>Animais Processado</span><input className="legacy-input" value={dadosGerais.animaisProcessado} onChange={(event) => setDadosGerais((prev) => ({ ...prev, animaisProcessado: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field"><span>Peso Identificação (KG)</span><input className="legacy-input" value={dadosGerais.pesoIdentificacaoKg} onChange={(event) => setDadosGerais((prev) => ({ ...prev, pesoIdentificacaoKg: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field"><span>Peso Bruto por Cabeça</span><input className="legacy-input" value={dadosGerais.pesoBrutoCabeca} onChange={(event) => setDadosGerais((prev) => ({ ...prev, pesoBrutoCabeca: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field" data-editavel="true"><span>RC Entrada</span><input className="legacy-input" value={dadosGerais.rcEntrada} onChange={(event) => setDadosGerais((prev) => ({ ...prev, rcEntrada: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field"><span>Peso com RC</span><input className="legacy-input" value={dadosGerais.pesoComRc} onChange={(event) => setDadosGerais((prev) => ({ ...prev, pesoComRc: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field"><span>Peso Considerado (@)</span><input className="legacy-input" value={dadosGerais.pesoConsideradoArroba} onChange={(event) => setDadosGerais((prev) => ({ ...prev, pesoConsideradoArroba: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field"><span>Peso Considerado (KG)</span><input className="legacy-input" value={dadosGerais.pesoConsideradoKg} onChange={(event) => setDadosGerais((prev) => ({ ...prev, pesoConsideradoKg: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field" data-editavel="true"><span>Peso Médio Abate</span><input className="legacy-input" value={dadosGerais.pesoMedioAbate} onChange={(event) => setDadosGerais((prev) => ({ ...prev, pesoMedioAbate: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field" data-editavel="true"><span>D.L.S.</span><input type="date" className="legacy-input" value={dadosGerais.dls} onChange={(event) => setDadosGerais((prev) => ({ ...prev, dls: event.target.value }))} /></label>
                <label className="legacy-field" data-editavel="true"><span>G.M.D.</span><input className="legacy-input" value={dadosGerais.gmd} onChange={(event) => setDadosGerais((prev) => ({ ...prev, gmd: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field" data-editavel="true"><span>G.M.C.</span><input className="legacy-input" value={dadosGerais.gmc} onChange={(event) => setDadosGerais((prev) => ({ ...prev, gmc: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field" data-editavel="true"><span>Consumo %P.V.</span><input className="legacy-input" value={dadosGerais.consumoPercentualPv} onChange={(event) => setDadosGerais((prev) => ({ ...prev, consumoPercentualPv: onlyDecimal(event.target.value) }))} /></label>
                <label className="legacy-field" data-editavel="true">
                  <span>Categoria</span>
                  <select
                    className="legacy-select"
                    value={dadosGerais.categoria}
                    onChange={(event) => setDadosGerais((prev) => ({ ...prev, categoria: event.target.value }))}
                  >
                    ...
                    {dadosGerais.categoria &&
                      !CATEGORIA_OPTIONS.includes(dadosGerais.categoria) && (
                        <option value={dadosGerais.categoria}>{dadosGerais.categoria}</option>
                      )}
                  </select>
                </label>
                <label className="legacy-field" data-editavel="true">
                  ...
                  <select
                    className="legacy-select"
                    value={dadosGerais.racaPredominante}
                    onChange={(event) => setDadosGerais((prev) => ({ ...prev, racaPredominante: event.target.value }))}
                  >
                    <option value="">Selecione...</option>
                    {RACA_PREDOMINANTE_OPTIONS.map((raca) => (
                      <option key={raca} value={raca}>
                        {raca}
                      </option>
                    ))}
                    {dadosGerais.racaPredominante &&
                      !RACA_PREDOMINANTE_OPTIONS.includes(dadosGerais.racaPredominante) && (
                        <option value={dadosGerais.racaPredominante}>{dadosGerais.racaPredominante}</option>
                      )}
                  </select>
                </label>
                <label className="legacy-check" data-editavel="true"><input type="checkbox" checked={dadosGerais.precoVendaFutura} onChange={(event) => setDadosGerais((prev) => ({ ...prev, precoVendaFutura: event.target.checked }))} />Preço Venda Futura</label>
                <section className="col-span-4 mt-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="section-title">Mapas</p>
                    <Link href={mapaPesagemHref} className="legacy-btn">Abrir módulo de mapa</Link>
                  </div>
                  <TabTable rows={mapas} onRemove={(index) => setMapas((prev) => prev.filter((_, i) => i !== index))} />
                </section>
              </div>
            )}
            {activeTab === "custos" && (
              <CustosTab
                resumo={custosResumo}
                onResumoChange={(field, value) => setCustosResumo((prev) => ({ ...prev, [field]: value }))}
                rows={custos}
                onAdd={() => openModal("custo")}
                onRemove={(index) => setCustos((prev) => prev.filter((_, i) => i !== index))}
              />
            )}
            {activeTab === "outros" && (
...
function toDadosGeraisPayload(dadosGerais: DadosGeraisForm) {
  return {
