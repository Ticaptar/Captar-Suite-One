...
import type { ContratoSaidaAnimaisListItem, ContratoStatus } from "@/lib/types/contrato";

type ListResponse = {
  items: ContratoSaidaAnimaisListItem[];
  total: number;
  page: number;
  pageSize: number;
};
...
export default function ContratoSaidaAnimaisListPage() {
  const router = useRouter();
  const [items, setItems] = useState<ContratoSaidaAnimaisListItem[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/contratos/saida-animais?${params.toString()}`, {
        cache: "no-store",
      });
  ...
  return (
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        <FormPageHeader
          title="Contratos de Saída de Animais"
          ...
          <div className="legacy-toolbar compact-toolbar">
            <div className="legacy-toolbar-left">
              <div className="legacy-actions">
                <Link href="/contratos/saida-animais/novo" className="legacy-btn primary">
                  Criar
                </Link>
              ...
              <tbody>
                ...
                {!loading &&
                  items.map((item) => (
                    <tr
                      key={item.id}
                      onClick={() => router.push(`/contratos/saida-animais/novo?id=${item.id}`)}
...
function tipoLabel(tipo: string): string {
  if (tipo === "saida_animais") return "Saída de Animais";
  return tipo;
}
