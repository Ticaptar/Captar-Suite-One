...
import {
  changeContratoSaidaAnimaisStatus,
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    if (body.gerarPedido && body.status === "ativo") {
      const contratoAtual = await getContratoSaidaAnimaisById(contratoId);
      if (!contratoAtual) {
    ...
    const updated = await changeContratoSaidaAnimaisStatus(contratoId, {
      status: body.status,
