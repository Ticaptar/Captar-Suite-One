...
import {
  getPesagemEntradaInsumosById,
  updatePesagemEntradaInsumos,
} from "@/lib/repositories/pesagens-entrada-insumos-repo";
...
export async function GET(_: Request, { params }: Params) {
  ...
  try {
    const data = await getPesagemEntradaInsumosById(parsedId);
    if (!data) {
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  try {
    const current = await getPesagemEntradaInsumosById(parsedId);
    if (!current) {
      return NextResponse.json({ error: "Pesagem nao encontrada." }, { status: 404 });
    }
    ...
    const updated = await updatePesagemEntradaInsumos(parsedId, {
      status: fluxo.status,
      ...
      classificacaoRows: body.classificacaoRows !== undefined ? parseClassificacaoRows(body.classificacaoRows) : undefined,
...
function parseClassificacaoRows(value: unknown) {
  if (!Array.isArray(value)) return [];
  return value
    .filter((item) => item && typeof item === "object" && !Array.isArray(item))
    .map((item) => {
      const row = item as Record<string, unknown>;
      const unidade = toOptionalNullableString(row.unidade) ?? null;
      return {
        tipoAnalise: String(row.tipoAnalise ?? "").trim(),
        valorEncontrado: parseOptionalNumber(row.valorEncontrado) ?? 0,
        pesoDesconto: parseOptionalNumber(row.pesoDesconto) ?? 0,
        unidade,
      };
    });
}
...
function parseFechamento(value: unknown) {
  ...
  return {
    ...
    talhao: toOptionalNullableString(row.talhao),
    pesoSecador: parseOptionalNumber(row.pesoSecador) ?? 0,
    pesoDescontoClassificado: parseOptionalNumber(row.pesoDescontoClassificado) ?? 0,
    pesoLiquidoDesconto: parseOptionalNumber(row.pesoLiquidoDesconto) ?? 0,
    pesoNotaFiscal: parseOptionalNumber(row.pesoNotaFiscal) ?? 0,
    pesoOrigem: parseOptionalNumber(row.pesoOrigem) ?? 0,
    numeroLaudo: toOptionalNullableString(row.numeroLaudo),
    armazenagemSilo: toOptionalNullableString(row.armazenagemSilo),
