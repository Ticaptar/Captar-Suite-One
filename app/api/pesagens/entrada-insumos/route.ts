...
import {
  createPesagemEntradaInsumos,
  listPesagensEntradaInsumos,
...
export async function GET(request: Request) {
  ...
  try {
    const data = await listPesagensEntradaInsumos({
      status,
      search,
      page,
      pageSize,
...
export async function POST(request: Request) {
  ...
  try {
    const created = await createPesagemEntradaInsumos({
      status: fluxo.status,
      ...
      classificacaoRows: parseClassificacaoRows(body.classificacaoRows),
  ...
  return value
    ...
    .map((item) => {
      ...
      return {
        ...
        unidade: toOptionalNullableString(row.unidade),
      };
    });
}
