...
import {
  getPesagemSaidaInsumosById,
  updatePesagemSaidaInsumos,
} from "@/lib/repositories/pesagens-saida-insumos-repo";
...
export async function GET(_: Request, { params }: Params) {
  ...
  try {
    const data = await getPesagemSaidaInsumosById(parsedId);
    if (!data) {
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  try {
    const current = await getPesagemSaidaInsumosById(parsedId);
    if (!current) {
      return NextResponse.json({ error: "Pesagem nao encontrada." }, { status: 404 });
    }
    ...
    const updated = await updatePesagemSaidaInsumos(parsedId, {
      status: fluxo.status,
