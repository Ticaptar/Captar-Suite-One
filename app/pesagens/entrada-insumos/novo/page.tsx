...
export default function PesagemEntradaInsumosFormPage() {
    ...
    async function loadOpcoes() {
      ...
      try {
        ...
        const [pesagemOpcoesRes, equipamentosRes, empresasRes, catalogRes] = await Promise.all([
          fetch(`/api/pesagens/entrada-insumos/opcoes?contratoLimit=120&_ts=${ts}`, { cache: "no-store" }),
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `/api/pesagens/entrada-insumos/opcoes?contratoLimit=140&contratoSearch=${encodeURIComponent(search)}&_ts=${ts}`,
          { cache: "no-store" },
        );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `/api/pesagens/entrada-insumos?page=1&pageSize=120&search=${encodeURIComponent(search)}&_ts=${ts}`,
          { cache: "no-store" },
        );
        if (!response.ok) return;
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      setError("");
      try {
        const response = await fetch(`/api/pesagens/entrada-insumos/${editingId}`, { cache: "no-store" });
  ...
  async function capturarPeso(target: "pesoBruto" | "pesoTara") {
    ...
    try {
      const response = await fetch("/api/pesagens/entrada-insumos/capturar-peso", { cache: "no-store" });
      if (!response.ok) {
  ...
  async function salvar() {
    ...
    try {
      ...
      const response = await fetch(editingId ? `/api/pesagens/entrada-insumos/${editingId}` : "/api/pesagens/entrada-insumos", {
      ...
      setTimeout(() => router.push(`/pesagens/entrada-insumos?${editingId ? "updated" : "created"}=${data.id ?? editingId ?? ""}`), 700);
    } catch (e) {
  ...
  return (
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        <FormPageHeader title={editingId ? `Pesagem #${editingId}` : "Pesagem de Caminhão de Entrada de Insumos"} subtitle="Cadastro de entrada de insumos com pesos e eventos." backHref="/pesagens/entrada-insumos" backLabel="Pesagens" />
        <ModuleHeader />
        <section className="card p-3">
          <div className="legacy-actions"><button type="button" className="legacy-btn primary" onClick={salvar} disabled={saving || loading}>{saving ? "Salvando..." : "Salvar"}</button><button type="button" className="legacy-btn" onClick={() => router.push("/pesagens/entrada-insumos")}>Cancelar</button></div>
...
function ClassificacaoListEditor({ rows, setRows }: { rows: ClassificacaoRow[]; setRows: (rows: ClassificacaoRow[]) => void }) {
  ...
  return (
    <div className="mt-2 space-y-2">
            ...
            {rows.map((row, index) => (
              <tr key={`${row.tipoAnalise || "analise"}-${index}`}>
                <td className="left">
                  <input
                    className="legacy-input"
                    list="entrada-insumos-tipo-analise"
                ...
                <td className="left">
                  <input
                    className="legacy-input"
                    list="entrada-insumos-unidade-analise"
                    value={row.unidade}
                    placeholder="Digite para buscar..."
                    onChange={(event) =>
      ...
      <datalist id="entrada-insumos-tipo-analise">
        {TIPO_ANALISE_OPTIONS.map((option) => (
          <option key={option} value={option} />
        ))}
      </datalist>
      <datalist id="entrada-insumos-unidade-analise">
    ...
    <div className="mt-2">
      <div className="legacy-grid cols-5">
        ...
        <label className="legacy-field col-span-2">
          ...
          <input
            className="legacy-input"
            list="entrada-insumos-armazenagem-silo"
      ...
      <datalist id="entrada-insumos-armazenagem-silo">
