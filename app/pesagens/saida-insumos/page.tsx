...
export default function PesagemSaidaInsumosListPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/pesagens/saida-insumos?${params.toString()}`, {
        cache: "no-store",
      });
      if (!response.ok) {
  ...
  return (
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        <FormPageHeader
          title="Pesagem de Caminhão de Saída de Insumos"
          ...
          <div className="legacy-toolbar compact-toolbar">
            <div className="legacy-toolbar-left">
              <div className="legacy-actions">
                <Link href="/pesagens/saida-insumos/novo" className="legacy-btn primary">
                  Criar
                </Link>
              ...
              <tbody>
                ...
                {!loading &&
                  items.map((item) => (
                    <tr
                      key={item.id}
                      onClick={() => router.push(`/pesagens/saida-insumos/novo?id=${item.id}`)}
