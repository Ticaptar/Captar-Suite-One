...
import type { PesagemSaidaAnimaisListItem, PesagemStatus } from "@/lib/types/pesagem";

type ListResponse = {
  items: PesagemSaidaAnimaisListItem[];
  total: number;
  page: number;
  pageSize: number;
};
...
export default function PesagemSaidaAnimaisListPage() {
  const router = useRouter();
  const [items, setItems] = useState<PesagemSaidaAnimaisListItem[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/pesagens/saida-animais?${params.toString()}`, {
        cache: "no-store",
      });
      if (!response.ok) {
  ...
  return (
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        <FormPageHeader
          title="Pesagem de Caminhão de Saída de Animal"
          ...
          <div className="legacy-toolbar compact-toolbar">
            <div className="legacy-toolbar-left">
              <div className="legacy-actions">
                <Link href="/pesagens/saida-animais/novo" className="legacy-btn primary">
                  Criar
                </Link>
              ...
              <tbody>
                ...
                {!loading &&
                  items.map((item) => (
                    <tr
                      key={item.id}
                      onClick={() => router.push(`/pesagens/saida-animais/novo?id=${item.id}`)}
