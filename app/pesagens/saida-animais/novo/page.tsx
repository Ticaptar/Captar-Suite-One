...
export default function PesagemSaidaAnimaisFormPage() {
    ...
    async function loadOpcoes() {
      ...
      try {
        ...
        const [pesagemOpcoesRes, equipamentosRes, empresasRes] = await Promise.all([
          fetch(`/api/pesagens/saida-animais/opcoes?contratoLimit=120&_ts=${ts}`, { cache: "no-store" }),
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `/api/pesagens/saida-animais/opcoes?contratoLimit=140&contratoSearch=${encodeURIComponent(search)}&_ts=${ts}`,
          { cache: "no-store" },
        );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          `/api/pesagens/saida-animais?page=1&pageSize=120&search=${encodeURIComponent(search)}&_ts=${ts}`,
          { cache: "no-store" },
        );
        if (!response.ok) return;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const ts = Date.now();
        const response = await fetch(`/api/contratos/saida-animais/${contratoId}?_ts=${ts}`, {
          cache: "no-store",
        });
        if (!response.ok) return;
        const data = (await response.json()) as ContratoMapaResponse;
        const linkedFromContrato = parseContratoLinkedGtaRows(data.contrato);
        const linkedFromCadastro = await fetchCadastroLinkedGtaRows(contratoId, "saida").catch(() => []);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      setError("");
      try {
        const response = await fetch(`/api/pesagens/saida-animais/${editingId}`, { cache: "no-store" });
  ...
  async function capturarPeso(target: "pesoBruto" | "pesoTara") {
    ...
    try {
      const response = await fetch("/api/pesagens/saida-animais/capturar-peso", { cache: "no-store" });
      if (!response.ok) {
  ...
  async function salvar() {
    ...
    try {
      ...
      const response = await fetch(editingId ? `/api/pesagens/saida-animais/${editingId}` : "/api/pesagens/saida-animais", {
      ...
      setTimeout(() => router.push(`/pesagens/saida-animais?${editingId ? "updated" : "created"}=${data.id ?? editingId ?? ""}`), 700);
    } catch (e) {
  ...
  return (
    <div className="page-shell min-h-screen px-2 py-2 md:px-3">
      <main className="w-full space-y-2">
        <FormPageHeader title={editingId ? `Pesagem #${editingId}` : "Pesagem de Caminhão de Saída de Animal"} subtitle="Cadastro de saída de animais com pesos e eventos." backHref="/pesagens/saida-animais" backLabel="Pesagens" />
        <ModuleHeader />
        <section className="card p-3">
          <div className="legacy-actions"><button type="button" className="legacy-btn primary" onClick={salvar} disabled={saving || loading}>{saving ? "Salvando..." : "Salvar"}</button><button type="button" className="legacy-btn" onClick={() => router.push("/pesagens/saida-animais")}>Cancelar</button></div>
